...
function list() {
  ...
  if (action.type === _constants.LIST_COMPLETE) {
    var newState = _.assign({}, action.meta, action.payload);
...
function details() {
  ...
  if ((action.type == _constants.DETAILS_COMPLETE || action.type == _constants.SAVE_COMPLETE) && action.payload._id) {
    ...
    var _key = _meta.service + '-' + _meta.model;
    var data = action.payload;
    return _.assign({}, state, (0, _defineProperty3.default)({}, _key, _.assign({}, state[_key], (0, _defineProperty3.default)({}, data._id, data))));
  }
  return state;
}
