  ...
  (0, _createClass3.default)(Editor, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      ...
      if (nextProps.remove && nextProps.remove._r == this._r) {
        var _t = this.context.t;
        this.loading = false;
        if (nextProps.remove.error) {
          //保存失败
          this.props.actions.notice(_t('Remove failed', { msg: nextProps.remove.error.message }));
        } else {
          this.props.actions.notice(_t('Removed successfully'));
    ...
    value: function init() {
      ...
      if (id === '_new') {
        var _ret = function () {
          var data = {};
          _lodash2.default.forEach(state.model.fields, function (field) {
    ...
    value: function handleChange(key, value) {
      this.setState({
        data: _lodash2.default.assign({}, this.state.data, (0, _defineProperty3.default)({}, key, value))
      });
    }
  }, {
    key: 'render',
    value: function render() {
      ...
      var title = t(model.label || model.name, serviceId) + ' > ';
      if (id == '_new') {
        title += t('Create');
      } else if (model.title) {
      ...
      for (var key in model.fields) {
        ...
        if (cfg.depends) {
          ...
          } else if (!_lodash2.default.every(cfg.depends, function (value, k) {
            return data[k] === value;
          })) {
            //没有全部匹配
            continue;
        ...
        if (model.noedit || this.loading || !canSave) {
          ...
          } else if (_lodash2.default.every(cfg.disabled, function (value, k) {
            return data[k] === value;
          })) {
            //全部匹配
            disabled = true;
      ...
      for (var _groupKey in groups) {
        ...
        var wrappers = _lodash2.default.get(views, path);
        if (wrappers) {
      ...
      if (id != '_new' && model.relationships && model.relationships.length) {
        relationships = _lodash2.default.map(model.relationships, function (r, index) {
...
var _initialiseProps = function _initialiseProps() {
  ...
  this.handleClose = function () {
    ...
    _this4.setState({ showRemoveDialog: false });
  };

  this.handleCreate = function () {
    var url = '/edit/' + _this4.state.serviceId + '/' + _this4.state.modelName + '/_new';
    _this4.context.router.replace(url);
  };

  this.handleRemove = function () {
    _this4._r = Math.random();
    _this4.props.actions.remove({
      service: _this4.state.serviceId,
      model: _this4.state.modelName,
      id: _this4.state.id,
      _r: _this4._r
    });
    _this4.loading = true;
  ...
  this.handleSave = function () {
    ...
    for (var key in fields) {
      ...
      if (field.required && !data[key]) {
        if (field.required === true || typeof field.required === 'string' && data[field.required] || (0, _typeof3.default)(field.required) === 'object' && _lodash2.default.every(field.required, function (value, k) {
          return data[k] === value;
        })) {
          var t = _this4.context.t;
    ...
    _this4.props.actions.save({
      service: model.service.id,
      model: model.name,
      key: model.key,
      _r: _this4._r,
      data: _lodash2.default.assign({}, data, { id: id == '_new' ? '' : id })
    });
  };

  this.remove = function () {
    _this4.setState({ showRemoveDialog: true });
  };
};
